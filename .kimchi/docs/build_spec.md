...
- Candidate eval model: `llama-3.3-70b-versatile` (70B — reasoning quality is the dominant signal; token budget is not binding at ~300 candidates post hard-requirement pre-filter)
- Batch size: 8 candidates per API call
- Sleep between batches: 2 seconds
- Max retries: 5

## Embedding Configuration
- Model: `sentence-transformers/all-mpnet-base-v2` (768-dim; upgraded from all-MiniLM-L6-v2 for better semantic discrimination of technical/ML content)
- Output dim: 768
- Normalize embeddings: True (L2)
- Batch size: 64 (CPU; mpnet uses more RAM than MiniLM)
- Model cache size: ~420 MB (one-time download to `~/.cache/huggingface/hub/`; Phase B never loads the model)

## Reasoning Format
Each row's `reasoning` column follows the exact format produced by Step 9:
`<current_title> with <X.X> yrs; <N> AI core skills; response rate <R.RR>.`

Example: `Senior ML Engineer with 6.5 yrs; 12 AI core skills; response rate 0.85.`

The fields are:
- `current_title`: candidate's profile.current_title (exact)
- `X.X`: candidate's profile.years_of_experience
- `N`: pre-computed count of evidenced AI/ML skills (endorsements > 0 OR duration > 0, name matches AI_CORE_SKILL_KEYWORDS)
- `R.RR`: candidate's redrob_signals.recruiter_response_rate (2 decimals)

The format is generated by the LLM in Step 9 and frozen — it flows unchanged through assemble_ranking.py, rank.py, and into the final CSV. No downstream step modifies it.
